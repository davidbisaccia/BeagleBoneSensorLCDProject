...
I2CDevice::I2CDevice(unsigned int bus, unsigned int device) 
		: _file(NULL), _bus(bus), _device(device), _open(false)
{
	Open();
}
...
bool I2CDevice::Open()
{
	string name = (_bus == 0) ? BBB_I2C_0 : BBB_I2C_1;

	if((_file = open(name.c_str(), O_RDWR)) < 0)
	{
	...
	if(ioctl(_file, I2C_SLAVE, _device) < 0)
	{
 ...
 * @return 1 on failure to write, 0 on success.
 */
bool I2CDevice::Write(unsigned char value)
{
...
unsigned char* I2CDevice::ReadRegisters(unsigned int number, unsigned int fromAddress)
{
	...
	if( read(_file, data, number) != (int)number)
	{
